...
export default function MyPlanPage() {
  ...
  const completedExercises = todayPlan.filter((item) => item.isDone).length;
  ...
  // Feature 3: Export/Share workout summary to clipboard
  const handleCopySummary = async () => {
    if (todayPlan.length === 0) {
      toast.info("Add some lifts to today's plan first!");
      return;
    }

    const lines = [
      "💪 FITLOG WORKOUT SESSION",
      `📅 ${new Date().toLocaleDateString("en-US", {
        weekday: "short",
        month: "short",
        day: "numeric",
      })}`,
      `📊 Completed: ${completedExercises}/${totalExercises} lifts (${progressPercent}%)`,
      `⏱️ Total Time: ${totalMinutes} min | 🔥 Est. Calories: ${totalCalories} kcal`,
      "",
      "LIFTS:",
      ...todayPlan.map(
        (w, i) =>
          `${i + 1}. ${w.name} (${w.equipment}) — ${w.sets} sets x ${
            w.reps
          } reps [${w.isDone ? "✓ COMPLETED" : "○ PENDING"}]`
      ),
      "",
      "Train with intent. Log every set — via FitLog",
    ];
  ...
  // Clear completed lifts
  const handleClearCompleted = () => {
    const completed = todayPlan.filter((w) => w.isDone);
    if (completed.length === 0) {
      toast.info("No completed lifts to clear.");
      return;
    }
    completed.forEach((w) => removeFromTodayPlan(w.id));
    toast.success(`Cleared ${completed.length} completed lift(s)!`);
  };
    ...
    <div className="py-10 md:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          ...
          {/* Feature 3 Action Buttons: Share & Clear */}
          {activeTab === "today" && todayPlan.length > 0 && (
            <div className="flex items-center gap-2.5">
              <button
                onClick={handleCopySummary}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-dark-card border border-dark-border hover:border-accent text-zinc-300 hover:text-white text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-sm"
              ...
              {completedExercises > 0 && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-dark-card border border-dark-border hover:border-red-500/40 text-zinc-400 hover:text-red-400 text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer"
                  title="Clear finished lifts"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
        ...
        {/* Feature 3: Interactive Session Progress Bar */}
        {activeTab === "today" && todayPlan.length > 0 && (
          <div className="bg-dark-card border border-dark-border rounded-2xl p-5 mb-8 shadow-md">
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="font-bold uppercase tracking-wider text-zinc-300">
                Session Progress
              </span>
              <span className="font-extrabold text-accent">
                {completedExercises} of {totalExercises} Lifts Finished (
                {progressPercent}%)
              </span>
            </div>
            ...
            {progressPercent === 100 && (
              <div className="mt-3 text-xs font-bold text-green-400 flex items-center gap-1.5">
                <span>🏆</span>
                <span>
                  All lifts finished! Outstanding effort in today&apos;s workout.
                </span>
              </div>
            )}
          </div>
